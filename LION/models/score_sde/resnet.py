...
class Prior(nn.Module):
    ...
    def __init__(self, args, in_channels, *oargs, **kwargs):
        super().__init__()
        # args: cfg.sde
        ...
        self.input_shape = (in_channels, 1, 1)

        self.nf = nf = 2 * in_channels # args.num_channels_dae
        ...
        if self.mixed_prediction:
            ...
            #    init = args.mixing_logit_init * torch.ones(size=[1, in_channels, 1, 1])
        ...
        modules = []
        self.input_layer = nn.Conv2d(in_channels, nf, 1, 1)
        in_ch = nf
        for i_block in range(args.num_cell_per_scale_dae):
            modules.append(self.building_block(nf, nf))
        self.output_layer = nn.Conv2d(nf, in_channels, 1, 1)
