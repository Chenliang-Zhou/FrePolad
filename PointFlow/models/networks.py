...
class PointFlow(nn.Module):
    def __init__(self, args):
        ...
        self.high_freq_recon_coeff = args.high_freq_recon_coeff
        self.high_freq_recon_lmax = args.high_freq_recon_lmax

        # kl annealing schedule
        kl_annealing_ratio = 0.5
        if args.cyclic_kl_annealing:
            num_cycles = 4
            self.kl_coeffs = [min(
                max(args.min_kl_weight + (args.max_kl_weight - args.min_kl_weight) * step / (kl_annealing_ratio * (args.num_epochs // num_cycles)),
                    args.min_kl_weight),
                args.max_kl_weight) for step in range(args.num_epochs // num_cycles)] * num_cycles
            self.kl_coeffs += [args.max_kl_weight] * (args.num_epochs - len(self.kl_coeffs))
        else:
            self.kl_coeffs = [min(
                max(args.min_kl_weight + (args.max_kl_weight - args.min_kl_weight) * step / (kl_annealing_ratio * args.num_epochs),
                    args.min_kl_weight),
                args.max_kl_weight) for step in range(args.num_epochs)]

        # print("KL coeffs:", self.kl_coeffs)

    @staticmethod
    def sample_gaussian(size, device, truncate_std=None):
    ...
    def forward(self, x, step, writer=None):
        ...
        y, delta_log_py = self.point_cnf(x, z, torch.zeros(batch_size, num_points, 1).to(x))
        log_py = standard_normal_logprob(y).view(batch_size, -1).sum(1, keepdim=True)
        delta_log_py = delta_log_py.view(batch_size, num_points, 1).sum(1)
        log_px = log_py - delta_log_py

        kl_weight = self.kl_coeffs[step - 1]  # -1 because step starts from 1
        # compute KL loss
        if kl_weight > 0:
            std = torch.exp(0.5 * z_sigma)
            std_log = z_sigma / 2
            kl_loss = (0.5 * std ** 2 + 0.5 * z_mu ** 2 - std_log - 0.5).sum(dim=1).mean()
        else:
            kl_loss = 0
        ...
        if self.high_freq_recon_coeff > 0:
            fre_loss_item = fre_loss(x, y, lmax=self.high_freq_recon_lmax) * 10 ** 7
    ...
    def decode(self, z, num_points=2048, truncate_std=None):
        ...
        y = self.sample_gaussian((z.size(0), num_points, self.input_dim), self.device, truncate_std)
    ...
    def sample(self, batch_size, num_points, truncate_std=None, truncate_std_latent=None):
        ...
        w = self.sample_gaussian((batch_size, self.zdim), self.device, truncate_std_latent)
        # z = self.latent_cnf(w, None, reverse=True).view(*w.size())
        # Sample points conditioned on the shape code
        y = self.sample_gaussian((batch_size, num_points, self.input_dim), self.device, truncate_std)
    ...
    def reconstruct(self, x, num_points=None, truncate_std=None):
        ...
        z = self.encode(x)
        return self.decode(z, num_points, truncate_std)
